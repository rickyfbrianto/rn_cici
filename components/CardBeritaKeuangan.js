...
const CardBeritaKeuangan = ({ style, batas, showControl = false }) => {
	...
	return (
		<View style={{ flex: 1 }}>
			<FlatList scrollEnabled={false} data={dataQuery.data} showsVerticalScrollIndicator={false} numColumns={2} columnWrapperStyle={{ columnGap: wp(3), flexWrap: "wrap" }} keyExtractor={(item, index) => index + item} refreshControl={<RefreshControl refreshing={refresh} onRefresh={handleRefresh} />} ListEmptyComponent={<CardEmpty refresh={handleRefresh} />} renderItem={({ item, index }) => <CardItem index={index} item={item} showControl={showControl} />} />
		</View>
	);
};
...
const CardItem = ({ item, index, showControl }) => {
	...
	return (
		<Swipeable ref={swipeRef} renderLeftActions={LeftSwipe}>
			...
			<Pressable onPress={() => router.push({ pathname: "berita/keuangan/[id]", params: { id: item.id } })} style={{ width: wp(44), columnGap: 5, marginTop: wp(4), borderRadius: 10, backgroundColor: "#edebd7" }}>
				<Text className="rounded-tl-lg rounded-br-lg" style={{ fontFamily: "outfit", zIndex: 10, width: wp(8), textAlign: "center", backgroundColor: COLORS.PRIMARY, height: 40, alignItems: "center", color: "white", position: "absolute", top: 0, left: 0, padding: 10 }}>
					{index + 1}
				</Text>
				...
				<View style={{ backgroundColor: "white", padding: wp(2), borderBottomLeftRadius: 10, borderBottomRightRadius: 10 }}>
