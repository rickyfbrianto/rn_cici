...
const IbadahCard = ({style, batas, showControl = false}) => {
    ...
    const dataQuery = useQuery({
        queryKey: ['ibadahList'],
        queryFn: async () => {
            const queryRef = batas ? query(ibadahCol, orderBy("tanggal"), limit(batas)) : query(ibadahCol, orderBy("tanggal"))
                    ...
                    ?   <SectionList sections={ConvertDataToSection(dataQuery.data)} showsVerticalScrollIndicator={false} keyExtractor={(item,index) => item+index} 
                        refreshControl={<RefreshControl refreshing={refresh} onRefresh={handleRefresh}/>}
                            renderItem={({item, index}) => (
                                <Pressable onPress={()=>{router.push(`${name}/${item.id}`)}} key={index} className="flex-row items-center gap-x-3 my-2 bg-white p-3 rounded-3xl">
